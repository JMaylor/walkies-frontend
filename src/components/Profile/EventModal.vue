<template>
	<b-modal ref="event-modal" @hidden="$emit('close-modal')" static>
		...
		<form>
			...
			<b-form-group label="Date" label-for="meetup-date-input" invalid-feedback="Pleae choose a date.">
				<b-form-datepicker :disabled="!formEnabled" id="meetup-date-input" v-model="meetupDate"></b-form-datepicker>
			</b-form-group>
			<b-form-group label="Time" label-for="meetup-time-input" invalid-feedback="Pleae choose a time.">
		...
		<template v-slot:modal-footer>
			<b-container>
				<b-row align-h="around">
					...
					>{{ (event.status == 'accepted' || event.proposer._id.$oid == $store.state.userProfile._id.$oid) ? 'Cancel Event' : 'Decline Invite' }}</b-button>
					<b-button @click="updateEvent" v-if="formEnabled" variant="outline-info">Send request</b-button>
					<b-button
						@click="acceptEvent"
						v-if="event.status == 'pending' && !formEnabled && $store.state.userProfile._id.$oid == event.invited._id.$oid"
...
<script>
	...
	export default {
		...
		computed: {
			otherUser() {
				return this.event.proposer._id.$oid == this.$store.state.userProfile._id.$oid ? this.event.invited : this.event.proposer;
			}
		},
		methods: {
			resetForm() {
