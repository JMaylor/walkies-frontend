  ...
  <b-card :header-bg-variant="colour">
    ...
    <b-card-body>
      ...
      <a
        target="_blank"
        :href="
          `https://www.google.com/maps?saddr=${userProfile.location.coordinates[1]},${userProfile.location.coordinates[0]}&daddr=${event.location.coordinates[1]},${event.location.coordinates[0]}`
        "
        >Get directions</a
      >
    </b-card-body>
...
export default {
  ...
  computed: {
    ...
    otherUser() {
      return this.userProfile._id.$oid === this.event.invited._id.$oid
        ? this.event.proposer
        : this.event.invited;
    },
    colour() {
