<template>
  <b-container>
    ...
    <!-- Dog Modal -->
    <DogModal
      v-if="addDogModalVisible"
      v-on:close-modal="addDogModalVisible = false"
    />

    <b-tabs fill content-class="mt-3" active-nav-item-class="text-info">
      <!-- Confirmed Events -->
      <b-tab :title="`Confirmed (${userConfirmedEvents.length})`">
        <b-row class="p-3">
          <h4 v-if="userConfirmedEvents.length > 0">
            Here are your upcoming walks!
          </h4>
        ...
        <b-row class="p-3" v-if="userConfirmedEvents.length > 0">
          ...
          >
            <Event
              v-bind:event="event"
              v-on:open-modal="openEventModal(event)"
            />
          </b-col>
        </b-row>
      </b-tab>
      ...
      <!-- Invites -->
      <b-tab :title="`Invites (${userInvitedEvents.length})`">
        <b-row class="p-3">
          <h4>
            {{
              userInvitedEvents.length > 0
                ? "Invites received from other users"
                : "No invites received :("
            }}
          </h4>
        </b-row>
        <b-row class="p-3" v-if="userInvitedEvents.length > 0">
          <b-col
            ...
            md="4"
            lg="3"
            v-for="event in userInvitedEvents"
            :key="event._id.$oid"
          >
            <Event
              v-bind:event="event"
      ...
      <!-- Pending -->
      <b-tab :title="`Pending (${userAwaitingEvents.length})`">
        <b-row class="p-3">
          <h4>
            {{
              userAwaitingEvents.length > 0
                ? "Pending response from other user"
        ...
        <b-row class="p-3" v-if="userAwaitingEvents.length > 0">
          <b-col
            ...
            md="4"
            lg="3"
            v-for="event in userAwaitingEvents"
            :key="event._id.$oid"
          >
            <Event
              v-bind:event="event"
      ...
      <!-- Dogs -->
      <b-tab :title="`Your dogs (${$store.state.userProfile.dogs.length})`">
        <!-- Add Dog Button -->
        <b-row class="p-3">
          <b-col>
            <b-btn
              @click="addDogModalVisible = true"
              v-on:close-modal="addDogModalVisible = false"
              >Add new dog</b-btn
            >
          </b-col>
        ...
        <b-row class="p-3">
          <b-col
            ...
            md="4"
            lg="3"
            v-for="dog in this.$store.state.userProfile.dogs"
            :key="dog._id.$oid"
          >
            <Dog v-bind:dog="dog" />
          </b-col>
